...
export async function saveDuck (duck) {
...
export function saveToUsersLikes (uid, duckId) {
  return Promise.all([ref.child(`usersLikes/${uid}/${duckId}`).set(true)])
}

export function deleteFromUsersLikes (uid, duckId) {
  return Promise.all([ref.child(`usersLikes/${uid}/${duckId}`).set(null)])
}

export function incrementNumberOfLikes (duckId) {
  return Promise.all([ref.child(`likeCount/${duckId}`).transaction((currentValue = 0) => currentValue + 1)])
}
